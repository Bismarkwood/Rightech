...
import { type RetailerOrder, type RetailerOrderItem } from '../../../core/data/mockRetailerOrders';
...
export function RetailerOrderDetailsDrawer() {
  ...
  const updateItem = (index: number, patch: Partial<RetailerOrderItem>) => {
    ...
    const prev = items[index] ?? { productId: '', name: '', qty: 1, unitPrice: '0 GHS', lineTotal: '0 GHS', image: '' };
  ...
  const addItem = () => {
    const items = [...(draft?.items ?? order?.items ?? [])];
    items.push({ productId: '', name: 'New item', qty: 1, unitPrice: '0 GHS', lineTotal: '0 GHS', image: '' });
    setItems(items);
    recalcTotals(items);
  };
