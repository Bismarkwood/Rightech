...
import { StorefrontEditorModal } from '../components/StorefrontEditorModal';
        ...
        <AnimatePresence mode="wait">
          {activeView === 'settings' ? (
            ...
            >
              ...
              {/* Product Cards Grid — Catalog Style */}
                  ...
                  {filteredListings.length > 0 ? (
                    filteredListings.map((listing) => {
                      ...
                      const statusColor = isOutOfStock
                        ...
                        : !listing.isPublished
                        ? 'text-[#525866] bg-[#F1F3F5] border-[#ECEDEF]'
                      ...
                      return (
                        <motion.div
                          ...
                          className="bg-white rounded-[22px] border border-[#ECEDEF] p-4 group hover:border-[#D40073] transition-all"
                        >
                          {/* Square image */}
                          <div className="aspect-square rounded-[14px] bg-[#F7F7F8] mb-4 overflow-hidden relative">
                            {listing.images.length > 0 ? (
                              <img
                                ...
                                className={`w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 ${!listing.isPublished || isOutOfStock ? 'grayscale' : ''}`}
                              />
                            ) : (
                            ...
                            {/* Status badge */}
                            <div className={`absolute top-2 right-2 px-2 py-0.5 rounded-[6px] text-[10px] font-bold border ${statusColor}`}>
                              {displayStatus}
                            </div>
                            {listing.isFeatured && listing.isPublished && !isOutOfStock && (
                              <div className="absolute top-2 left-2 bg-[#D40073] text-white px-2 py-0.5 rounded-[6px] text-[10px] font-bold">
                                Featured
                              </div>
                            )}
                            {/* Edit overlay */}
                          ...
                          {/* Text */}
                          <h3 className="text-[14px] font-bold text-[#111111] mb-1 truncate">{listing.displayName}</h3>
                          <p className="text-[12px] text-[#525866] mb-3 truncate">{listing.storeCategory}</p>
                          <div className="flex items-center justify-between">
                            <p className={`text-[14px] font-black ${isOutOfStock ? 'text-[#8B93A7] line-through' : 'text-[#D40073]'}`}>
                              GHS {liveItem.price.toLocaleString()}
                            </p>
                            <p className="text-[12px] font-bold text-[#111111]">{liveItem.stock} in stock</p>
                          </div>
                        </motion.div>
                      );
                    })
